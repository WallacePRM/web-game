...
const Sprite = (filename) => ({
  ...
  runningFront: [
    { x: 0, y: 0 - FaceIndex.runningFront * SPRITE_HEIGTH },
    { x: 0 - SPRITE_WIDTH, y: 0 - FaceIndex.runningFront * SPRITE_HEIGTH },
  ],
  runningBack: [
    { x: 0, y: 0 - FaceIndex.runningBack * SPRITE_HEIGTH },
    { x: 0 - SPRITE_WIDTH, y: 0 - FaceIndex.runningBack * SPRITE_HEIGTH },
  ],
  runningLeft: [
    { x: 0, y: 0 - FaceIndex.runningLeft * SPRITE_HEIGTH },
    { x: 0 - SPRITE_WIDTH, y: 0 - FaceIndex.runningLeft * SPRITE_HEIGTH },
  ],
  runningRight: [
    { x: 0, y: 0 - FaceIndex.runningRight * SPRITE_HEIGTH },
    { x: 0 - SPRITE_WIDTH, y: 0 - FaceIndex.runningRight * SPRITE_HEIGTH },
  ],
});
...
class SpritesController {
   ...
   */
  setView = (faceName) => {
    if (this._currentFace === faceName) return;
  ...
  updateView = () => {
    ...
    if (Array.isArray(face)) {
      ...
      this._faceIndex++;
      if (this._faceIndex >= face.length) this._faceIndex = 0;
    } else this._updateSprite(face);
  };
